...
public class SoundManager {
    ...
    private RobbojuniorActivity _application;
    ...
    private final Set<String> custom_sounds_list;

    public SoundManager(RobbojuniorActivity application) {
        ...
        try {
            soundList = new HashSet<String>(listCustomSounds(application));
        } catch (IOException e) {
            Log.e(LOG_TAG, "Could not retrieve list of custom sounds from application", e);
            soundList= Collections.emptySet();
        }
        custom_sounds_list =  soundList;
    }
    ...
    public synchronized int playSound(String file) {
        ...
        try {
            ...
            else  if ((_html5AssetList.contains(file)) || ( custom_sounds_list).contains(file)) {
                final String path = "HTML5/sounds/" + file;
     ...
     * 
     * @paramid The id of the sound to stop. If already stopped, does nothing.
     */
    ...
    private void loadSoundEffects() {
        if (_soundEffectPool == null) {
            ...
            try {
                String[] soundEffects = assetManager.list("HTML5/sound_effects");
                loadSoundEffects(assetManager, "HTML5/sound_effects/", soundEffects);
    ...
    private List<String> listHTML5Assets(RobbojuniorActivity application)
        throws IOException
    {
    ...
    private List<String> listCustomSounds(RobbojuniorActivity application)
            throws IOException
    {
        ArrayList<String> result = new ArrayList<String>();
      //  result.addAll(Arrays.asList(application.getAssets().list("HTML5")));
        for (String path : application.getAssets().list("HTML5/sounds")) {
            result.add(path);
        }
        return result;
    }
